...
void ScoreWindow::calculatePoints(QStringList str3)//计算绩点
{
    ...
    finalGPA = QString("平均绩点：%1").arg(GPA2);
...
void ScoreWindow::updateInfoWindow(QStringList)
{
    ...
    QString text ="<style>"".p1{font-size:20px;}""</style>"
    ...
    ui->textEdit->setCurrentFont(QFont("楷体",20));
...
void ScoreWindow::on_importButton_clicked()
{
    ...
    if(!db.open())
    {
        ...
        for(int i=0;i<4;i++){
            select_sql = QString("select courseID from t_courses where courseName='%1';").arg(singledata.at(a+i));
        ...
        for(int i=1;i<tempText.size();i++)
        {
            ...
            qDebug()<<singledata;
            int a=5;
            for(int j=0;j<singledata.size()-a;j++)
            {
                select_sql = QString("insert into t_stud_course_info value ('','%1','%2','%3','','');").arg(singledata.at(1),signalcourseID.at(j),singledata.at(a+j));
                qDebug()<<singledata.at(1)<<signalcourseID.at(j)<<singledata.at(a+j);
                if(!sql_query.exec(select_sql))
                {
                    qDebug()<<sql_query.lastError();
                }
...
//    database.setDatabaseName("D:/database.db");
