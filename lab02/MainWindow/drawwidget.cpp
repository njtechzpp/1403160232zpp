...
DrawWidget::DrawWidget(QWidget *parent) : QWidget(parent)
{
    ...
    img->fill (BACKGROUNDOVER_COLOR);
    pix = new QPixmap(size());      //此QPixmap对象用来准备随时接受绘制的内容
    pix->fill (BACKGROUNDOVER_COLOR);          //填充背景画布为白色
...
//    painter.drawPixmap ( this->width()/4 , this->height()/4 , this->width()/2 , this->height()/2, *img);
...
//    painter.drawPixmap ( width()/4 , height()/4 , width()/2 , height()/2, *img);
...
void DrawWidget::resizeEvent (QResizeEvent *event)
{
    ...
    {
        QPixmap *newPix = new QPixmap(size());
        newPix->fill (BACKGROUNDOVER_COLOR);
        QPainter p(newPix);
...
void DrawWidget::clear ()
{
    ...
    // 发现背景画布白色会覆盖之后添加的背景，于是改用背景覆盖透明色
    pix->fill(BACKGROUNDOVER_COLOR);
    img->fill(BACKGROUNDOVER_COLOR);
    update ();
}
...
void DrawWidget::drawPic()
{
    QString Filename;
     Filename = QFileDialog::getOpenFileName(this,
                                           tr("选择图像"),
                                           "D:/github/1403160232ZPP/lab02/picture",
