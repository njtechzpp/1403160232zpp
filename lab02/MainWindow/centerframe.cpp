...
void CenterFrame::createUserCommandArea()
{
   ...
   QPointF pt2(p.size().width()/2,1);
   QPointF pt3(p.size().width()-3,p.size().height()/2);
   QPointF pt4(p.size().width()/2,p.size().height()-1);
   QVector<QPointF> pts;
   ...
   p.fill(BACKGROUND_COLOR);
   // 三角形的三个顶点
   pt1 = QPointF(3,p.size().height()-3);
   pt2 = QPointF(p.size().width()/2,3);
   pt3 = QPointF(-3+p.size().width(),-3+p.size().height());
   pts.clear();
